...
_ASSESSMENT_TRACE_ID_PATTERN = re.compile(r"{assessment\.trace_id}")
...
def _convert_path_parameter_to_flask_format(path):
    ...
    """
    # Handle Databricks-specific syntax like {assessment.trace_id} -> <trace_id> first for efficiency
    # (very specific/rare compared to the general catch-all pattern)
    path = _ASSESSMENT_TRACE_ID_PATTERN.sub(r"<trace_id>", path)
    # Handle simple parameters like {trace_id}
    return _SIMPLE_PARAM_PATTERN.sub(r"<\1>", path)

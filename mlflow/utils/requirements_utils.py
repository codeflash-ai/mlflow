...
def _get_pinned_requirement(req_str, version=None, module=None):
    ...
    package = req.name
    if version is None:
        version_raw = _cached_get_installed_version(package, module)
        ...
        if local_version_label:
            ...
            # Use cached check for Databricks runtime
            if not (_cached_is_in_databricks_runtime() and package in ("torch", "torchvision")):
                msg = (
...
def _cached_get_installed_version(package, module):
    # Reuse original function via cache
    return _get_installed_version(package, module)


@lru_cache(maxsize=1)
def _cached_is_in_databricks_runtime():
    # This is expensive only once per session.
    return is_in_databricks_runtime()
